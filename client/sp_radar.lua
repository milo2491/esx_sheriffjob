...
function radarSetSpeed(defaultText)
    ...
    if (GetOnscreenKeyboardResult()) then
        ...
        if gettxt ~= nil then
            ...
            AddTextComponentString("~r~Please enter the correct number !")
...
function sheriff_radar()
    ...
    if isRadarPlaced then 
        ...
        if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), RadarPos.x, RadarPos.y, RadarPos.z, true) < 0.9 then 
            ...
            ClearPrints()
            SetTextEntry_2("STRING")
            AddTextComponentString("~r~You're not off the radar !")
            DrawSubtitleTimed(3000, 1)
    ...
    while true do
        ...
        if isRadarPlaced then
            ...
            if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), RadarPos.x, RadarPos.y, RadarPos.z, true) > 300 then 
                ...
                ClearPrints()
                SetTextEntry_2("STRING")
                AddTextComponentString("~r~You're too far off your radar !")
                DrawSubtitleTimed(3000, 1)
        ...
        if isRadarPlaced then
            ...
            if veh ~= nil then
                ...
                if vehSpeedKm < maxSpeed then
                    info = string.format("   vehicle ~r~%s ~w~Mark ~r~%s ~w~Km/h ~g~%s", vehDesc, vehPlate, math.ceil(vehSpeedKm))
                else
                    info = string.format("   vehicle ~r~%s ~w~Mark ~r~%s ~w~Km/h ~r~%s", vehDesc, vehPlate, math.ceil(vehSpeedKm))
                    ...
                    LastInfo = string.format("   vehicle ~r~%s ~w~Mark ~r~%s ~w~Km/h ~r~%s", LastVehDesc, LastPlate, math.ceil(LastSpeed))
                end
